...
class LinkedDatabasesData(ZileanSys):
    ...
    @zileanmoves(__file__, LinkedDatabasesData)
    def _link_database(self, db, local=True):
        l = 1 if local else 0
    ...
    @classmethod
    def database_data(cls, db):
        obj = object.__new__(cls)
        obj = obj.__init__()
        return obj.database_data(db)

    @classmethod
    def link_database(cls, db, local=True):
        obj = object.__new__(cls)
